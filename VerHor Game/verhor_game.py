...
def create_verhor_game(rows, cols):
    row = [False for _ in range(cols)]
    board = [row.copy() for _ in range(rows)]
    return VerHorGame(board.copy())


class VerHorGame(object):
    ...
    def is_legal_move(self, row, col, vertical):
        if vertical:
            verhor = ((row, col), (row+1, col))
        else:
            verhor = ((row, col), (row, col+1))

        if not self.move_on_board(verhor):
            return False

        if not self.move_on_free_space(verhor):
            return False

        return True

    def move_on_board(self, verhor):
        for square in verhor:
            row = square[0]
            col = square[1]
    ...
    def move_on_free_space(self, verhor):
        for square in verhor:
            row = square[0]
            col = square[1]
    ...
    def copy(self):
        return VerHorGame(copy.deepcopy(self._board))
